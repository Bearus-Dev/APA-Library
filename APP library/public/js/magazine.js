...
class Art{
    constructor(Autor, Year, Titulo, Place, Editorial, Url){
        this.Autor = Autor;
        this.Year = Year;
        this.Titulo = Titulo;
        this.Place= Place;
...
class UI {
    addproduct(art){
        ...
        <div class="card">
                <div class="card-body">
                    <strong> Bibliografia</strong>: ${art.Autor}. (${art.Year}). ${art.Titulo}. ${art.Place}, de  ${art.Editorial}. Recuperado de: ${art.Url}
                </div>
        </div>
        `;
...
//Obtiene el dato desde el html
    ...
    const art = new Art(Autor, Year, Titulo, Place, Editorial, Url);
    const ui = new UI();
    ui.addproduct(art);
    ui.resetForm();
...
const response = await db.collection("apabook").doc().set({
    Autor,
    Year,
    Titulo,
    Place,
